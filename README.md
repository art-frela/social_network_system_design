...
Размер 1 записи + Индексы + выравнивание (только PK) \~[Byte]

| Сущность\таблица   | Таблица        | Тип БД            | Описание схемы                                     | Размер 1 записи | Число записей в год | Прирост в год | Операций в секунду (I/O) | Интенсивность трафика |
| ------------------ | -------------- | ----------------- | -------------------------------------------------- | --------------- | ------------------- | ------------- | ------------------------ | --------------------- |
| Пользователи       | users          | RDBMS             | [database/main_rdms.dbml](database/main_rdms.dbml) | 137+40          | 10000000            | 1,77GB        | 12                       | 2124Bps               |
| Подписки           | subscriptions  | RDBMS             | [database/main_rdms.dbml](database/main_rdms.dbml) | 40+40           | 252288000           | 20,18GB       | 16                       | 4800Bps               |
| Локации            | locations      | RDBMS             | [database/main_rdms.dbml](database/main_rdms.dbml) | 83+40           | 2000000             | 0,25GB        | 12                       | 1476Bps               |
| Места              | places         | RDBMS             | [database/main_rdms.dbml](database/main_rdms.dbml) | 949+40          | 1000000             | 1GB           | 12                       | 11868Bps              |
| Ссылки на фото     | photos         | RDBMS             | [database/main_rdms.dbml](database/main_rdms.dbml) | 151+40          | 1135296000          | 217GB         | 36                       | 6876Bps               |
| Посты              | posts          | RDBMS             | [database/main_rdms.dbml](database/main_rdms.dbml) | 2835+40         | 378432000           | 1088GB        | 12                       | 34500Bps              |
...
| Фотографии         |                | Object storage    | [database/main_blob.dbml](database/main_blob.dbml) | _512016_        | 1892160000          | 969816GB      | 180                      | 92MBps                |
...
| ----------------- | --------------------------- | ------------------ | ---------------------------------------------- |
| RDBMS             | 6386,3GB→6,3TB              | 376                | 222036Bps                                      |
...
Disks = max(
  ceil(Disks_for_capacity),
  ceil(Disks_for_throughput),
  ceil(Disks_for_iops)
);
```
...
| хар-ка\БД            | RDBMS (HDD 2Tb 100Mbps 100 iops) | Object storage (HDD 36Tb 100Mbps 100 iops) | Key-Value Storage (HDD 128Gb 100Mbps 100 iops) |
| -------------------- | -------------------------------- | ------------------------------------------ | ---------------------------------------------- |
| disks_for_capacity   | 6,2TB/2TB=3,1                    | 970TB/36TB=26.9                            | 12,1GB/128GB=0,094                             |
...
| DISKS                | 4 -> 6 (raid5 или типа того)     | 27 + 20% -> 32                             | 2 (raid1 для надежности)                       |
